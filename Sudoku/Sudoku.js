(function(){
  ...
  const rankDiff = document.getElementById('rank-diff');
  ...
  function onLevelCompleted(){
    ...
    // Al completar, limpiar progreso en curso y guardar resultado
    saveProgress(state.user, {completed, bonusLives, inProgress: null});

    state.baseLives = BASE_LIVES + bonusLives;
    state.lives = state.baseLives;
    showToast('Progreso guardado');
    // Actualizar ranking en UI después de guardar
    if(rankDiff) renderLeaderboard(rankDiff.value || state.difficulty);
  }
  ...
  function loadProgress(user){
    try{
      ...
      if(!raw) return {completed:{}, bonusLives:0, inProgress:null};
      const obj = JSON.parse(raw);
      return {completed: obj.completed||{}, bonusLives: obj.bonusLives||0, inProgress: obj.inProgress||null};
    }catch(e){ return {completed:{}, bonusLives:0, inProgress:null}; }
  }
  function saveProgress(user, data){
    ...
    const merged = {
      ...
      inProgress: data && ('inProgress' in data) ? data.inProgress : obj.inProgress
    };
  ...
  function getAllProgressEntries(){
    ...
    for(let i=0;i<localStorage.length;i++){
      ...
      if(!k || !k.startsWith('sudokuxx:')) continue;
      try{
        const user = k.slice('sudokuxx:'.length);
        const obj = JSON.parse(localStorage.getItem(k)||'{}');
        res.push({user, data: obj});
      }catch(e){/* ignore */}
    }
    return res;
  }
  function getLeaderboardForDifficulty(diff){
    const entries = getAllProgressEntries();
    const rows = [];
    for(const {user, data} of entries){
      const times = (data.completed && data.completed[diff]) || [];
      if(times.length){
        const best = times.reduce((m,t)=>Math.min(m, t.timeMs||0), Infinity);
        if(Number.isFinite(best)) rows.push({user, bestMs: best});
      }
    }
  ...
  function renderLeaderboard(diff){
    ...
    const rows = getLeaderboardForDifficulty(diff);
    leaderboard.innerHTML = '';
    rows.forEach((r, idx)=>{
      const li = document.createElement('li');
      li.textContent = `#${idx+1} ${r.user} — ${formatTime(r.bestMs)}`;
  ...
  function saveOnExit(){
    ...
    const inProgress = {
      ...
      lives: state.lives,
      // Guardar estado del tablero y lista de puzzles para poder reanudar exactamente
      currentBoard: state.current ? state.current.board.slice() : null,
      puzzles: state.puzzles && state.puzzles.length ? JSON.parse(JSON.stringify(state.puzzles)) : [],
      date: Date.now()
    };
    saveProgress(state.user, {inProgress});
  }

  function resumeFromProgress(p){
    state.difficulty = p.difficulty || state.difficulty;
    state.puzzles = Array.isArray(p.puzzles) && p.puzzles.length ? p.puzzles : [];
    state.sudokuIndex = p.sudokuIndex || 0;
    state.baseLives = BASE_LIVES + (loadProgress(state.user).bonusLives || 0);
    state.lives = (p.lives!=null) ? p.lives : state.baseLives;
    if(state.puzzles.length){
      state.current = JSON.parse(JSON.stringify(state.puzzles[state.sudokuIndex]));
      state.selectedIdx = null;
      state.current.board = Array.isArray(p.currentBoard) && p.currentBoard.length===81 ? p.currentBoard.slice() : state.current.givens.slice();
      renderBoard(state.current);
      if(state.isAdmin){ renderSolution(state.current.solution); }
      updateHUD();
    }
  ...
  btnEmpezarNivel.addEventListener('click', ()=>{
    ...
    // Si hay progreso en curso en esta dificultad, retomamos el tiempo y el tablero; si no, iniciamos de cero.
    const prog = loadProgress(state.user);
    if(prog.inProgress && prog.inProgress.difficulty===state.difficulty){
      state.sessionElapsedMs = prog.inProgress.sessionElapsedMs||0;
      uiTime.textContent = formatTime(state.sessionElapsedMs);
      resumeFromProgress(prog.inProgress);
      startTimer();
      gameView.querySelector('.selectors').classList.add('hidden');
      showToast('Progreso retomado');
      return;
    }
  ...
  if(btnGuardar){
    btnGuardar.addEventListener('click', ()=>{ saveOnExit(); showToast('Progreso guardado'); });
  }
  if(btnLogout){
    btnLogout.addEventListener('click', ()=>{
      ...
      // Limpiar estado mínimo visible
      uiTime.textContent = '00:00';
      state.user = null;
    });
  }
  if(btnActualizarRanking && rankDiff){
    btnActualizarRanking.addEventListener('click', ()=>{
      renderLeaderboard(rankDiff.value);
    });
  }
  ...
  loginBtn.addEventListener('click', ()=>{
    ...
    // El tiempo no inicia al iniciar sesión; mostrar tiempo guardado si existe.
    const prog = loadProgress(state.user);
    state.sessionElapsedMs = (prog.inProgress && prog.inProgress.sessionElapsedMs) || 0;
    uiTime.textContent = formatTime(state.sessionElapsedMs);
    // Inicializar ranking UI
    if(rankDiff){ rankDiff.value = state.difficulty; }
    if(rankDiff) renderLeaderboard(rankDiff.value);
  });
